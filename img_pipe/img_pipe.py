...
class freeCoG:
    ...
    def reg_img(self, source='CT.nii', target='orig.mgz', smooth=0., reg_type='rigid', interp='pv', xtol=0.0001, ftol=0.0001, savehr=True):
        ...
        Update: 08-03-2022, Sophocles:

        savehr: True or False
        ...
        orig_highRes = os.path.join(self.mri_dir, 'orig_highRes.mgz')
        brain = os.path.join(self.mri_dir, 'brain.mgz')
        brain_highRes = os.path.join(self.mri_dir, 'brain_highRes.mgz')
    ...
    def label_elecs_hip(self, elecfile_prefix='elecs_all', atlas_surf='desikan-killiany', atlas_depth='destrieux', elecs_all=True):
        ...
        Produces 4 columns of output, corresponding to HBT, FS60, CA, and full setups.
        More information can be found here https://surfer.nmr.mgh.harvard.edu/fswiki/HippocampalSubfieldsAndNucleiOfAmygdala

        Important: 'subfield_scan' needs to be defined while initializing the patient.
        Assumes elecs_all.mat in elecs folder.
        Saves a new file with _hip.mat in the end.
        Parameters
        ----------
        elecfile_prefix : str, optional
            prefix of the .mat file with the electrode coordinates matrix
        ...
        if elecfile_prefix == 'TDT_elecs_all' or elecfile_prefix == 'clinical_elecs_all' or elecs_all:
            ...
            # Initialize the cell array that we'll store electrode labels in later
            elec_labels_orig = np.empty((len(long_label),7),dtype=np.object)
            elec_labels_orig[:,0] = short_label
            elec_labels_orig[:,1] = long_label
            elec_labels_orig[:,2] = grid_or_depth 

            elec_labels = np.empty((len(indices_to_use),7), dtype = np.object)
            elecmatrix_orig = elecmatrix
            elecmatrix = elecmatrix[indices_to_use,:]
        ...
        if np.any(np.invert(isnotdepth)): # If there are depth electrodes, run this part
            ...
            if(self.subfield_scan != 'None'):
                ...
                for i,atl in enumerate(atlases):
                    aseg_file = os.path.join(self.subj_dir, self.subj, 'mri', 'lh.hippoAmygLabels-' + self.subfield_scan + '.v21' + atl + 'FSvoxelSpace.mgz')
                    dat = nib.freesurfer.load(aseg_file)
                    aparc_dat_lh = dat.get_data()
                
                    aseg_file = os.path.join(self.subj_dir, self.subj, 'mri', 'rh.hippoAmygLabels-' + self.subfield_scan + '.v21' + atl + 'FSvoxelSpace.mgz')
                    dat = nib.freesurfer.load(aseg_file)
                    aparc_dat_rh = dat.get_data()
                    ...
                    elec_labels[np.invert(isnotdepth),3+i] = anatomy
                    print("Finished labelling with the %s set"%(atl))
        ...
        #make some corrections b/c of NaNs in elecmatrix
        elec_labels_orig[:,3:7] = ''
        elec_labels_orig[indices_to_use,3:7] = elec_labels[:,3:7] 
        
        print('Saving electrode labels to %s'%(elecfile_prefix))
        scipy.io.savemat(os.path.join(self.elecs_dir, elecfile_prefix+'_hip.mat'), {'elecmatrix': elecmatrix_orig, 
                                                                                'anatomy': elec_labels_orig})
